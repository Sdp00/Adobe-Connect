...
// NEW: reactive hook for Preact components
export function useCurrentUser() {
  const [userInfo, setUserInfo] = useState(getUserInfo());

  useEffect(() => {
    const refresh = () => setUserInfo(getUserInfo());
    window.addEventListener('user:ready', refresh);
    return () => window.removeEventListener('user:ready', refresh);
  }, []);
